  ...
  class Software
    ...
    def with_standard_compiler_flags(env = {}, opts = {})
      ...
      project.env_appends.each do |name, val|
        compiler_flags[name] = compiler_flags.has_key?(name) ? compiler_flags[name] + " " + val : val
      end

      project.env_prepends.each do |name, val|
        compiler_flags[name] = compiler_flags.has_key?(name) ? val + " " + compiler_flags[name] : val
      end

      project.env_overrides.each do |name, val|
        compiler_flags[name] = val.dup
      end
